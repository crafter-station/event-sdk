  ...
  if (submitted) {
    return (
      <div className="rounded-xl border border-[var(--accent)]/20 bg-[var(--accent)]/5 p-8 text-center">
        ...
        <p className="text-sm text-[var(--muted)]">Your project &quot;{data.title}&quot; has been submitted successfully.</p>
        <a href="/submissions" className="mt-4 inline-block text-sm text-[var(--accent)] hover:underline">
    ...
    <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {hackathon.tracks.map((track) => (
              ...
              >
                ...
                {track.description && <p className="text-[var(--muted)] line-clamp-1">{track.description}</p>}
              </button>
            ))}
          </div>
        </div>
      ...
      {hackathon?.judgingCriteria && (
        <div className="rounded-lg border border-white/5 bg-[var(--surface)] p-4">
          <p className="text-xs font-mono text-[var(--accent)] tracking-widest uppercase mb-3">Judging Criteria</p>
          <div className="space-y-2">
            {hackathon.judgingCriteria.map((c) => (
              <div key={c.name} className="flex items-center justify-between text-xs">
                <div>
                  <span className="font-medium">{c.name}</span>
                  {c.description && <span className="text-[var(--muted)] ml-2">— {c.description}</span>}
                </div>

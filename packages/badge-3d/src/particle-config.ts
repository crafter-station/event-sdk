...
export const particleShapes = ["sphere", "cube", "diamond", "capsule"] as const;

export const particleGroupSchema = z.object({
	color: z.string().describe("CSS hex color, e.g. '#ffd700'"),
	count: z.number().int().min(10).max(2500).describe("Number of particles in this group"),
	size: z.number().min(0.008).max(0.04).describe("Particle radius (0.012 is default)"),
	shape: z
		.enum(particleShapes)
		.describe(
			"Particle shape: sphere (round), cube (blocky), diamond (faceted gem), capsule (pill/elongated)",
		),
	metalness: z.number().min(0).max(1).describe("Material metalness (0=matte, 1=metal)"),
	roughness: z.number().min(0).max(1).describe("Material roughness (0=glossy, 1=rough)"),
	emissive: z.string().describe("Emissive hex color for glow, e.g. '#000000' for no glow"),
	emissiveIntensity: z
		.number()
		.min(0)
		.max(3)
		.describe("Emissive intensity (0=no glow, 2+=strong glow)"),
	clearcoat: z.number().min(0).max(1).describe("Clearcoat layer (0=none, 1=full glossy coat)"),
	opacity: z.number().min(0.15).max(1).describe("Opacity (1=solid, 0.3-0.6=fluid/glass look)"),
	transmission: z
		.number()
		.min(0)
		.max(1)

    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-[var(--background)]/80 backdrop-blur-xl">
      <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4">
        ...
        <div className="flex items-center gap-6">
          <Link href="/speakers" className="text-xs text-[var(--muted)] hover:text-white transition-colors">
            {t.nav.speakers}
          </Link>
          <Link href="/schedule" className="text-xs text-[var(--muted)] hover:text-white transition-colors">
            {t.nav.schedule}
          </Link>
          <Link href="/sponsors" className="text-xs text-[var(--muted)] hover:text-white transition-colors">
            {t.nav.sponsors}
          </Link>
          {event.features.jobs?.enabled && (
            <Link href="/jobs" className="text-xs text-[var(--muted)] hover:text-white transition-colors">
              {t.nav.jobs}
            </Link>
          )}
          {event.features.deck && (
            <Link href="/deck" className="text-xs text-[var(--accent)] hover:underline transition-colors">
              {t.nav.deck}
            </Link>
          )}

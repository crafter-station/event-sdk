  ...
  return (
    <section className="py-20 px-6">
      <div className="mx-auto max-w-4xl text-center">
        <p className="font-mono text-xs text-[var(--accent)] tracking-widest uppercase mb-4">{t.countdown.label}</p>
        <div className="flex items-center justify-center gap-4 md:gap-8">
          {units.map((u) => (
            <div key={u.label} className="flex flex-col items-center">
              ...
              <span className="mt-1 text-[10px] font-mono uppercase tracking-wider text-[var(--muted)]">{u.label}</span>
            </div>
          ))}
        </div>
      </div>
